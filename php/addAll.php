...
// --------------归属于Link.php的代码---------------------
      $appid ="wxc4ad29b9b72855c1"; 
      $appsecret ="1c74875ac8fab33d5ac3508b7a8808ab"; 
      ...
      if(empty($row) || $row['update_timestamp'] < time()){//不存在或失效 调接口
        ...
        print_r($result);
    if(empty($row)){
             $sqls = "insert into wx_sys(`access_token`,`update_timestamp`,`expires_in`,`appid`) values('$access_token','$update_timestamp','$access_token','$appid')";
             $result = $mysqli->query($sqls);
         }else{
             $id = $row['id'];
             $sqls = "update wx_sys set `access_token`='$access_token',`update_timestamp`='$update_timestamp' where `id`=$id ";
             $result = $mysqli->query($sqls);
         }
      }else{
